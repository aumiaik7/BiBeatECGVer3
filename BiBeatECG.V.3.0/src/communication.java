...
public class communication {
    ...
    public interface Hid extends WinNT {
        ...
        /**
         * Get Interrupt Report from device
         */

        boolean HidD_GetInputReport(WinNT.HANDLE HidDeviceObject, byte[] InReportBuffer, int InReportBufferLength);
    }
    ...
    //////////test Get Interrupt Report
     public byte GetInReport(byte[] buffer) {
        /* Check to see that the device is opened */
        if (HIDHandle.equals(WinBase.INVALID_HANDLE_VALUE)) {
            return HID_DEVICE_NOT_OPENED;
        }
        /* Get Feature report */
        boolean Status = Hid.INSTANCE.HidD_GetInputReport(HIDHandle, buffer, buffer.length);
        if (Status == false) {
            System.out.println("Fail");
            return HID_DEVICE_TRANSFER_FAILED;
